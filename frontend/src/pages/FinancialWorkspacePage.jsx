/**
 * GeM-Guard v6.0 — Financial Evaluator Workspace
...
export default function FinancialWorkspacePage() {
  ...
  async function handleResetDemo() {
    ...
    try {
      await resetDemo();
      setBanner({ type: 'success', msg: '✓ Demo data reset. Refreshing…' });
      await loadTenders();
    } catch (e) {
      setBanner({ type: 'error', msg: e.message });
    } finally {
          ...
          <div style={{ ...card, marginBottom: 16 }}>
            ...
            {loading ? (
              ...
              <div style={{ textAlign: 'center', padding: 24, color: C.muted, fontSize: 12 }}>
                No tenders found.<br />
                <button onClick={handleResetDemo} style={{ marginTop: 8, padding: '6px 12px', borderRadius: 6, background: C.goldBg, border: `1px solid ${C.goldBorder}`, color: C.gold, cursor: 'pointer', fontSize: 11 }}>
                  Seed Demo Data
                </button>
              </div>
            ) : (
