...
5. RBAC: All routes require FINANCIAL_EVALUATOR role (enforced at Gateway + validated here).
"""
...
async def upload_financial_envelope(
    ...
    if user_role and user_role not in ("BIDDER", "FINANCIAL_EVALUATOR", "PROCUREMENT_OFFICER"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only BIDDER, FINANCIAL_EVALUATOR, or PROCUREMENT_OFFICER may upload financial envelopes."
        )
...
async def unseal_financial_envelope(
    ...
    if user_role and user_role != "FINANCIAL_EVALUATOR":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=f"Financial envelope unsealing requires FINANCIAL_EVALUATOR role. Got: {user_role}",
        )
