...
class ComplianceEngine:
    ...
    def _apply_operator(cls, op: str, val: Any, thresh: Any, metric: str) -> Tuple[bool, str]:
        """Applies deterministic operator comparison."""
        # Handle Boolean threshold (e.g. active required)
        if isinstance(thresh, bool) or str(thresh).lower() in ("true", "false"):
            ...
            # If value is string "ACTIVE", "YES", etc.
            bool_val = True if str(val).upper() in ("ACTIVE", "YES", "TRUE", "VERIFIED", "1") or val else False
            passed = (bool_val == bool_thresh)
            return passed, f"Metric '{metric}' checked ({val} -> {bool_val} vs required {bool_thresh})."
        ...
        try:
            ...
            if op in (">=", "GTE"):
                ...
                return passed, f"Value {num_val} >= required {num_thresh} ({metric})."
            elif op in (">", "GT"):
                passed = num_val > num_thresh
                return passed, f"Value {num_val} > required {num_thresh} ({metric})."
            elif op in ("<=", "LTE"):
        ...
        str_thresh = str(thresh).strip().upper()

        if op in ("==", "EQ", "MATCH"):
            passed = str_val == str_thresh
            return passed, f"String '{str_val}' matched required '{str_thresh}'."
        elif op in ("!=", "NEQ"):
            passed = str_val != str_thresh
            return passed, f"String '{str_val}' != '{str_thresh}'."
        elif op in ("contains", "IN"):
            passed = str_thresh in str_val or str_val in str_thresh
            return passed, f"Value '{str_val}' contains '{str_thresh}'."
        elif op == "EXISTS":
            passed = bool(val and str(val).strip())
            return passed, f"Metric '{metric}' presence confirmed."

        return False, f"Unsupported operator '{op}' for metric '{metric}'."
